...
func (c *pectraG1MSM) RequiredGas(input []byte) uint64 {
	...
	return msmGasCost(k, pectraG1MSMBaseGas)
}
...
func (c *pectraG2MSM) RequiredGas(input []byte) uint64 {
	...
	return msmGasCost(k, pectraG2MSMBaseGas)
}
...
//
// The discount table is identical to the one libevm carries for
// draft EIP-2537 (it predates the Pectra-final spec but was used
// unchanged). discount(k) for k >= 128 is the table's last entry.
// ---------------------------------------------------------------

func msmGasCost(k, perPair uint64) uint64 {
	if k == 0 {
		return 0
	}
	idx := k - 1
	if idx >= uint64(len(ethparams.Bls12381MultiExpDiscountTable)) {
		idx = uint64(len(ethparams.Bls12381MultiExpDiscountTable) - 1)
	}
	discount := ethparams.Bls12381MultiExpDiscountTable[idx]
