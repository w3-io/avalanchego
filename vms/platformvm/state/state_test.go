...
func TestStateDiffIntegration_DeleteValidatorAndItsDelegator(t *testing.T) {
	state := newTestState(t, memdb.New())
...
// TestStateDiffIntegration_ReplaceValidatorAndUpdateStakingInfo tests validator replacement (delete and put) with a
// StakingInfo update because validator replacements are expected to be followed with some type of staking metadata
// update.
func TestStateDiffIntegration_ReplaceValidator(t *testing.T) {
	updateWeight := func(s *Staker) *Staker {
		updated := *s
		updated.Weight += 1
	...
	tests := []struct {
		name             string
		updateValidatorF func(s *Staker) *Staker
		wantStakingInfo  StakingInfo
	}{
		{
			// A no-op replacement is dropped entirely and does not interact with the underlying state
			name: "noop_update",
		},
		{
			name:            "noop_update_with_staking_info_set",
			wantStakingInfo: StakingInfo{DelegateeReward: 123},
		},
		{
			name:             "update",
			updateValidatorF: updateWeight,
		},
		{
			name:             "update_with_staking_info_set",
			updateValidatorF: updateWeight,
			wantStakingInfo:  StakingInfo{DelegateeReward: 123},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// In the next diff replace the validator
			diff, err = NewDiffOn(state, true)
			require.NoError(t, err)

			require.NoError(t, diff.DeleteCurrentValidator(validator))

			wantUpdated := validator
			if tt.updateValidatorF != nil {
				wantUpdated = tt.updateValidatorF(validator)
			}

			require.NoError(t, diff.PutCurrentValidator(wantUpdated))
			require.NoError(t, diff.SetStakingInfo(wantUpdated.SubnetID, wantUpdated.NodeID, tt.wantStakingInfo))
			require.NoError(t, diff.Apply(state))
			require.NoError(t, state.Commit())

			gotValidator, err := state.GetCurrentValidator(wantUpdated.SubnetID, wantUpdated.NodeID)
			require.NoError(t, err)
			require.Equal(t, wantUpdated, gotValidator)

			gotStakingInfo, err := state.GetStakingInfo(wantUpdated.SubnetID, wantUpdated.NodeID)
			require.NoError(t, err)
			require.Equal(t, tt.wantStakingInfo, gotStakingInfo)
		})
	}
}

// TestStateAndDiffIntegration_GetStakingInfo_DefaultsToZero tests that if a StakingInfo is not set, it defaults to the
// zero value.
func TestStateAndDiffIntegration_GetStakingInfo_DefaultsToZero(t *testing.T) {
	state := newTestState(t, memdb.New())
	...
	// We should default to zero if we did not explicitly set a value for StakingInfo
	got, err := state.GetStakingInfo(validator.SubnetID, validator.NodeID)
	require.NoError(t, err)
	require.Zero(t, got)
}

// TestStateAndDiffIntegration_AddValidatorThenSetStakingInfo tests the case where StakingInfo is set in a Diff after it
// was initially added.
func TestStateAndDiffIntegration_AddValidatorThenSetStakingInfo(t *testing.T) {
	state := newTestState(t, memdb.New())
	...
	// In the next diff set its StakingInfo
	want := StakingInfo{DelegateeReward: 123}
...
// TestStateAndDiffIntegration_AddValidatorWithStakingInfo tests the case where StakingInfo is set the same Diff that
// it is added in.
func TestStateAndDiffIntegration_AddValidatorWithStakingInfo(t *testing.T) {
	state := newTestState(t, memdb.New())
...
// TestStateAndDiffIntegration_AddValidatorWithStakingInfo tests that StakingInfo is deleted if a validator is deleted
// in a separate Diff.
func TestStateAndDiffIntegration_SetStakingInfoThenDeleteValidator(t *testing.T) {
	state := newTestState(t, memdb.New())
...
// Tests that StakingInfo is deleted if a validator is deleted in the same Diff.
func TestStateAndDiffIntegration_SetStakingInfoAndDeleteValidator(t *testing.T) {
	state := newTestState(t, memdb.New())
...
// TestStateAndDiffIntegration_DeleteValidatorAndAddDifferentValidatorWithStakingInfo tests deleting a validator and
// adding a different validator with StakingInfo set.
func TestStateAndDiffIntegration_DeleteValidatorAndAddDifferentValidatorWithStakingInfo(t *testing.T) {
	state := newTestState(t, memdb.New())
