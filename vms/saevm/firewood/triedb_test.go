...
func newSUT(t *testing.T) *SUT {
	cfg := DefaultConfig(t.TempDir(), saetest.NewTBLogger(t, logging.Debug))
...
func TestGenesis(t *testing.T) {
	dir := t.TempDir()
	cfg := DefaultConfig(dir, saetest.NewTBLogger(t, logging.Debug))
	memDB := rawdb.NewMemoryDatabase()
	tdb := triedb.NewDatabase(memDB, &triedb.Config{
		DBOverride: cfg.BackendConstructor,
	})

	keys := saetest.NewUNSAFEKeyChain(t, 10) // genesis must be non-empty
	g := core.Genesis{
	...
	t.Run("recovery", func(t *testing.T) {
		cfg.Log = saetest.NewTBLogger(t, logging.Debug)
		tdb = triedb.NewDatabase(memDB, &triedb.Config{
			DBOverride: cfg.BackendConstructor,
		})
